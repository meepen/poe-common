...
class IsAnyValidator implements ValidatorConstraintInterface {
    ...
    async validate(value: any, args: ValidationArguments): Promise<boolean> {
        const validators = args.constraints[0] as (new() => IsAnyWrapperInterface)[];
        ...
        if (isValid === undefined) {
            this.validationErrors.push(
                ...results
                    .map((result) => `${result.map((errors) => Object.entries(errors.constraints || {}).map(([key, value]) => `${key}: ${value}`).join(' and ')).join(' and ')}`)
            );
        }
...
export function IsAny(validators: PropertyDecorator[][], options?: ValidationOptions): PropertyDecorator {
    ...
    return (target: object, propertyName: string | symbol) => {
        if (typeof propertyName !== 'string') {
            throw new Error(`IsAny decorator can only be used on string properties, got ${typeof propertyName}`);
        }
